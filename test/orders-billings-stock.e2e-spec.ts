...
describe('Orders + Billings + StoreStock (e2e)', () => {
  ...
  it('should create order, billing and manage store stock', async () => {
    ...
    const customerRes = await request(app.getHttpServer()).post('/customers').send(customerPayload).expect(201)
    ...
    const productRes = await request(app.getHttpServer()).post('/product/create-product').send(productPayload).expect(201)
    ...
    const orderPayload = {
      ...
      items: [{ product_id: product.id, quantity: 2, unit_price: 1500, discount: 100 }],
      notes: 'first order',
    }
    const orderRes = await request(app.getHttpServer()).post('/orders').send(orderPayload)
    if (orderRes.status !== 201) {
      // Log validation error
      // eslint-disable-next-line no-console
      console.error('Order create error:', orderRes.status, JSON.stringify(orderRes.body, null, 2))
    }
    ...
    const billingRes = await request(app.getHttpServer()).post(`/orders/${order.id}/billings`).send(billingPayload).expect(201)
    expect(billingRes.body.order_id).toBe(order.id)

    // list billings for order
    const listRes = await request(app.getHttpServer()).get(`/orders/${order.id}/billings`).expect(200)
    expect(Array.isArray(listRes.body)).toBe(true)
    expect(listRes.body.length).toBeGreaterThanOrEqual(1)

    // upsert store stock for product
    const upsertRes = await request(app.getHttpServer()).patch(`/store-stock/${product.id}`).send({ quantity: 10 }).expect(200)
