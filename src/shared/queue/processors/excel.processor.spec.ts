/**
 * ExcelProcessor unit tests — see image.processor.spec for rationale.
 */
...
const makeJob = (name: string): Job =>
	({ id: 'j1', name, data: { outputPath: '/out.xlsx' }, attemptsMade: 1 }) as unknown as Job
...
describe('ExcelProcessor', () => {
	...
	beforeEach(async () => {
		...
		processor = module.get(ExcelProcessor)
		jest.clearAllMocks()
	})

	it('throws "not implemented" for any job (until real impl lands)', async () => {
		await expect(processor.process(makeJob('sales-report'))).rejects.toThrow(/not yet implemented/)
	})

	it('onFailed → routes to DLQ', async () => {
		const job = makeJob('sales-report')
