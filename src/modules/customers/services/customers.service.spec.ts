...
function makeP2002(target: string[]): Prisma.PrismaClientKnownRequestError {
	const err = new Error('Unique constraint') as unknown as Prisma.PrismaClientKnownRequestError & {
		code?: string
		meta?: any
	}
...
describe('CustomersService', () => {
	...
	beforeEach(async () => {
		const module = await Test.createTestingModule({
			providers: [CustomersService, { provide: CUSTOMER_REPOSITORY, useValue: customerRepository }],
		}).compile()
	...
	it('create should pass billing_day to repository', async () => {
		...
		expect(customerRepository.create).toHaveBeenCalledWith(
			expect.objectContaining({ billing_day: 15 }),
		)
