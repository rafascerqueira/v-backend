...
export class ReportsService {
	...
	async getFullReport(period: Period = "month") {
		...
		const { start: prevStart, end: prevEnd } =
			this.getPreviousDateRange(period);
	...
	private async getTopProducts(
		...
		const productStats: Record<
			string,
			{ name: string; vendas: number; receita: number }
		> = {};
	...
	async getGrowthMetrics() {
		...
		const [
			...
			this.prisma.order.aggregate({
				where: { ...tenantFilter, createdAt: { gte: currentMonthStart } },
				_sum: { total: true },
				_count: true,
			}),
			this.prisma.order.aggregate({
		...
		const revenueGrowth = previousMonthOrders._sum.total
			? (((currentMonthOrders._sum.total || 0) -
		...
		const ordersGrowth = previousMonthOrders._count
			? ((currentMonthOrders._count - previousMonthOrders._count) /
