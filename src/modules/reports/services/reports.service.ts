...
export class ReportsService {
	...
	async getFullReport(period: Period = 'month') {
		...
		const [currentOrders, previousOrders] = await Promise.all([
			this.reportsRepository.findOrdersWithItems(tenantFilter, { gte: start, lte: end }),
	...
	async getChartsData(period: Period = 'month') {
		const tenantFilter = this.getTenantFilter()
		const { start, end } = this.getDateRange(period)

		const orders = await this.reportsRepository.findOrdersForCharts(tenantFilter, {
			gte: start,
			lte: end,
		})
		...
		return {
			hourlyDistribution: hourlyData.map((count, hour) => ({
			...
			revenueTimeline: Object.entries(dailyRevenue)
	...
	async getGrowthMetrics() {
		...
		const [currentMonthOrders, previousMonthOrders, currentMonthCustomers, previousMonthCustomers] =
			await Promise.all([
				this.reportsRepository.aggregateOrders(tenantFilter, { gte: currentMonthStart }),
				this.reportsRepository.aggregateOrders(tenantFilter, {
					gte: previousMonthStart,
					lte: previousMonthEnd,
				}),
				this.reportsRepository.countCustomers(tenantFilter, { gte: currentMonthStart }),
				this.reportsRepository.countCustomers(tenantFilter, {
					gte: previousMonthStart,
					lte: previousMonthEnd,
				}),
			])

		const revenueGrowth = previousMonthOrders._sum.total
			? (((currentMonthOrders._sum.total || 0) - previousMonthOrders._sum.total) /
					previousMonthOrders._sum.total) *
				100
			: 0

		const ordersGrowth = previousMonthOrders._count
			? ((currentMonthOrders._count - previousMonthOrders._count) / previousMonthOrders._count) *
				100
			: 0
		...
		return {
			currentMonth: {
				revenue: currentMonthOrders._sum.total || 0,
				orders: currentMonthOrders._count,
				newCustomers: currentMonthCustomers,
			},
			previousMonth: {
				revenue: previousMonthOrders._sum.total || 0,
				orders: previousMonthOrders._count,
	...
	async getCustomersReport(limit = 10) {
		...
		const customerOrders = await this.reportsRepository.groupOrdersByCustomer(tenantFilter, limit)

		const customerIds = customerOrders.map((c) => c.customer_id)
		const customers = await this.reportsRepository.findCustomersByIds(customerIds)
		...
		const totalCustomers = await this.reportsRepository.countAllCustomers(tenantFilter)
		const activeCustomers = await this.reportsRepository.countActiveCustomers(tenantFilter)
