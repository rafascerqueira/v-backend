...
describe('StockMovementsService', () => {
  ...
  it('create IN movement should increase stock and create movement', async () => {
    tx.product.findUnique.mockResolvedValueOnce({ id: 1 })
    tx.store_stock.findUnique.mockResolvedValueOnce({ product_id: 1, quantity: 2, reserved_quantity: 0, min_stock: 0, max_stock: 0 })
    ...
    const res = await service.create({ movement_type: 'in', reference_type: 'purchase', reference_id: 1, product_id: 1, quantity: 5 })
  ...
  it('create OUT movement should decrease stock', async () => {
    tx.product.findUnique.mockResolvedValueOnce({ id: 1 })
    tx.store_stock.findUnique.mockResolvedValueOnce({ product_id: 1, quantity: 5, reserved_quantity: 0, min_stock: 0, max_stock: 0 })
    ...
    const res = await service.create({ movement_type: 'out', reference_type: 'sale', reference_id: 2, product_id: 1, quantity: 3 })
  ...
  it('should reject OUT when insufficient stock', async () => {
    tx.product.findUnique.mockResolvedValueOnce({ id: 1 })
    tx.store_stock.findUnique.mockResolvedValueOnce({ product_id: 1, quantity: 1, reserved_quantity: 0, min_stock: 0, max_stock: 0 })

    await expect(
      service.create({ movement_type: 'out', reference_type: 'sale', reference_id: 3, product_id: 1, quantity: 5 })
  ...
  it('should reject when product not found', async () => {
    ...
    await expect(
      service.create({ movement_type: 'in', reference_type: 'purchase', reference_id: 4, product_id: 9, quantity: 1 })
    ).rejects.toBeInstanceOf(BadRequestException)
  })
})
