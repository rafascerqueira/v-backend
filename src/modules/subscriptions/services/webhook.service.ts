...
export class WebhookService {
	...
	private async handlePagSeguroSubscriptionActive(
		data: PagSeguroWebhookEvent["data"],
	) {
		...
		if (subscription) {
			await this.prisma.subscription.update({
				...
				data: {
					status: "active",
					current_period_end: data.next_invoice_at
						? new Date(data.next_invoice_at)
						: undefined,
				},
			});
	...
	private async handlePagSeguroSubscriptionEnded(
		data: PagSeguroWebhookEvent["data"],
	) {
		const accountId = data.reference_id;
		if (!accountId) return;
	...
	private async handlePagSeguroPaymentFailed(
		data: PagSeguroWebhookEvent["data"],
	) {
		await this.prisma.subscription.updateMany({
			where: { provider_subscription_id: data.id },
