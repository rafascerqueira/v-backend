...
export class PlanGuard implements CanActivate {
	...
	async canActivate(context: ExecutionContext): Promise<boolean> {
		...
		const requiredFeature = this.reflector.getAllAndOverride<PlanFeatures>(REQUIRED_FEATURE_KEY, [
			context.getHandler(),
			context.getClass(),
		])

		if (requiredFeature) {
			const hasFeature = await this.subscriptionService.hasFeature(userId, requiredFeature)
			if (!hasFeature) {
				throw new ForbiddenException(
		...
		// Check usage limit
		const checkLimit = this.reflector.getAllAndOverride<LimitType>(CHECK_LIMIT_KEY, [
			context.getHandler(),
			context.getClass(),
		])

		if (checkLimit) {
			const limitCheck = await this.subscriptionService.checkLimit(userId, checkLimit)
