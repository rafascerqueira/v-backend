...
describe('BillingsService', () => {
  ...
  it('create should map dates and set order_id', async () => {
    prismaMock.order.findUnique.mockResolvedValueOnce({ id: 2, seller_id: 'test-seller-id' })
  ...
  it('update should map nullable dates', async () => {
    prismaMock.billing.findUnique.mockResolvedValueOnce({ id: 3, order: { seller_id: 'test-seller-id' } })
